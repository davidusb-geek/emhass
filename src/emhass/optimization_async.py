...
class Optimization:
    ...
    async def perform_optimization(
        ...
        """
        # Run the optimization in a thread pool to avoid blocking the event loop
        return await asyncio.get_event_loop().run_in_executor(
            None,
            self._perform_optimization_sync,
            data_opt,
            P_PV,
            P_load,
            unit_load_cost,
    ...
    def _perform_optimization_sync(
        self,
        data_opt: pd.DataFrame,
        P_PV: Any,
        P_load: Any,
        ...
        """
        Synchronous version of perform_optimization for use in thread pool.
        """
        ...
        if def_total_timestep is not None:
            def_total_hours = [0 if x != 0 else x for x in def_total_hours or []]
        ...
        def_total_hours = (def_total_hours or []) + [0] * (num_deferrable_loads - len(def_total_hours or []))
        def_start_timestep = (def_start_timestep or []) + [0] * (num_deferrable_loads - len(def_start_timestep or []))
        def_end_timestep = (def_end_timestep or []) + [0] * (num_deferrable_loads - len(def_end_timestep or []))
        ...
        if self.costfun == "profit":
            ...
            if type_self_conso == "bigm":
                ...
                ) if SC is not None else plp.lpSum(0 for i in set_I)
            else:
                self.logger.error("Not a valid option for type_self_conso parameter")
                objective = plp.lpSum(0 for i in set_I)
        else:
        ...
        if self.plant_conf["inverter_is_hybrid"]:
            constraints = {
                ...
                for i in set_I
            } if P_hybrid_inverter is not None else {}
        else:
        ...
        if self.plant_conf["inverter_is_hybrid"]:
            ...
            if P_hybrid_inverter is not None:
                constraints.update(
                    {
        ...
        if self.costfun == "self-consumption" and SC is not None and type_self_conso == "maxmin":
            constraints.update(
                {
        ...
        for k in range(self.optim_conf["number_of_deferrable_loads"]):
                ...
                if def_load_config and "thermal_config" in def_load_config:
                    ...
                    for Id in set_I:
                        ...
                        if len(desired_temperatures) > Id and desired_temperatures[Id]:
                            ...
                            if hasattr(opt_model, "objective"):
            ...
            # --- Standard/non-thermal deferrable load logic comes after thermal ---
                ...
                (def_total_timestep and len(def_total_timestep) > k and def_total_timestep[k] > 0)
                or (def_total_hours and len(def_total_hours) > k and def_total_hours[k] > 0)):

                self.logger.debug(f"Load {k} is standard/non-thermal.")
                if def_total_timestep and len(def_total_timestep) > k and def_total_timestep[k] > 0:
                    ...
                    hours_val = def_total_hours[k] if def_total_hours and len(def_total_hours) > k else 0
                    self.logger.debug(f"Load {k}: Using total hours constraint: {hours_val}")
                    constraints.update(
                        {
                            f"constraint_defload{k}_energy": plp.LpConstraint(
                                e=plp.lpSum(
            ...
            start_val = def_start_timestep[k] if def_start_timestep and len(def_start_timestep) > k else 0
            end_val = def_end_timestep[k] if def_end_timestep and len(def_end_timestep) > k else 0
            self.logger.debug(
                f"Deferrable load {k}: Proposed optimization window: {start_val} --> {end_val}"
            )
            if def_total_timestep and len(def_total_timestep) > k and def_total_timestep[k] > 0:
                def_start, def_end, warning = Optimization.validate_def_timewindow(
                    def_start_timestep[k] if def_start_timestep and len(def_start_timestep) > k else 0,
                    def_end_timestep[k] if def_end_timestep and len(def_end_timestep) > k else 0,
                    ceil(
            ...
            if (self.optim_conf.get("set_deferrable_load_single_constant", []) and
                len(self.optim_conf["set_deferrable_load_single_constant"]) > k and
                self.optim_conf["set_deferrable_load_single_constant"][k]):
            ...
            if (self.optim_conf.get("treat_deferrable_load_as_semi_cont", []) and
                len(self.optim_conf["treat_deferrable_load_as_semi_cont"]) > k and
                self.optim_conf["treat_deferrable_load_as_semi_cont"][k]):
                constraints.update(
                    {
            ...
            constraints.update(
                {
                    f"constraint_socfinal_{0}": plp.LpConstraint(
                        ...
                        sense=plp.LpConstraintEQ,
                        rhs=((soc_init or 0) - (soc_final or 0))
                        * self.plant_conf["battery_nominal_energy_capacity"]
                        / self.timeStep,
                    )
                }
        ...
        if self.optim_conf["set_use_battery"]:
            opt_tp["P_batt"] = [
                (P_sto_pos[i].varValue if hasattr(P_sto_pos[i], "varValue") else P_sto_pos[i]) +
                (P_sto_neg[i].varValue if hasattr(P_sto_neg[i], "varValue") else P_sto_neg[i]) for i in set_I
            ]
            SOC_opt_delta = [
                (
                    (P_sto_pos[i].varValue if hasattr(P_sto_pos[i], "varValue") else P_sto_pos[i])
                    * (1 / self.plant_conf["battery_discharge_efficiency"])
                    + self.plant_conf["battery_charge_efficiency"]
                    * (P_sto_neg[i].varValue if hasattr(P_sto_neg[i], "varValue") else P_sto_neg[i])
                )
        ...
        if self.plant_conf["inverter_is_hybrid"] and P_hybrid_inverter is not None:
        ...
        if self.costfun == "profit":
            ...
            if type_self_conso == "maxmin" and SC is not None:
        ...
        for day in self.days_list_tz:
            ...
            if day_start.tzinfo != day_end.tzinfo:
                ...
                try:
                    ...
                    self.logger.warning(f"Failed to generate date range for day {day}: {e}")
                    continue
    ...
    async def perform_dayahead_forecast_optim(
        ...
        # Call optimization function
        result = await self.perform_optimization(
            df_input_data,
            P_PV.values.ravel(),
            P_load.values.ravel(),
            unit_load_cost,
        ...
        self.opt_res = result if isinstance(result, pd.DataFrame) else pd.DataFrame()
        return self.opt_res
    ...
    def validate_def_timewindow(
        ...
        if start <= end or start <= 0 or end <= 0:
            ...
            if end_validated > 0 and (end_validated - start_validated) < min_steps:
