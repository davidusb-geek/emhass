...
class MLForecaster:
    r"""
    An async forecaster class using machine learning models with auto-regressive approach and features\
    ...
    async def fit(
        ...
        # print("data_exo", self.data_exo)

        # Check if var_model exists in data before assignment
        ...
        self.data_exo = await asyncio.to_thread(
            self.data_exo.interpolate, method="linear", axis=0, limit=None
        )
        # print("data_exo", self.data_exo)
        ...
        # print("date_train", self.date_train)
        # The last 5 days
        self.date_split = (
        ...
        # print(self.data_train[self.var_model])
        # print(self.data_train.drop(self.var_model, axis=1))
    ...
    async def predict(self, data_last_window: pd.DataFrame | None = None) -> pd.Series:
        ...
        if data_last_window is None:
            ...
            data_last_window = await asyncio.to_thread(
                data_last_window.interpolate, method="linear", axis=0, limit=None
            )
    ...
    async def tune(self, debug: bool | None = False) -> pd.DataFrame:
