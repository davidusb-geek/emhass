...
# note, its a good idea to remove the "llvm-dev" packadge and "LLVM_CONFIG=/usr/bin/llvm-config pip3 install 'llvmlite>=0.43'" once the llvmlite package has been fixed in piwheels
RUN [[ "${TARGETARCH}" == "armhf" || "${TARGETARCH}" == "armv7" ]] && LLVM_CONFIG=/usr/bin/llvm-config pip3 --no-cache-dir --break-system-packages  install 'llvmlite>=0.43' && pip3 install --index-url=https://www.piwheels.org/simple --no-cache-dir --break-system-packages -r requirements.txt ||  pip3 install --no-cache-dir --break-system-packages -r requirements.txt
