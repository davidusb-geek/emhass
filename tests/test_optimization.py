...
class TestOptimization(unittest.IsolatedAsyncioTestCase):
    ...
    def test_heatpump_group_mixed_semi_cont(self):
        """Test heatpump group with semi-cont underfloor heating and non-semi-cont hot water."""
        self.optim_conf["number_of_deferrable_loads"] = 2
        self.optim_conf["nominal_power_of_deferrable_loads"] = [1000, 2000]
        self.optim_conf["operating_hours_of_each_deferrable_load"] = [0, 0]
        # Load 0: semi-continuous (modulating 580-1000W)
        # Load 1: NOT semi-continuous (fixed 2000W or off)
        self.optim_conf["treat_deferrable_load_as_semi_cont"] = [True, False]
        self.optim_conf["set_deferrable_load_single_constant"] = [False, False]
        self.optim_conf["set_deferrable_startup_penalty"] = [0, 0]
        self.optim_conf["set_deferrable_max_startups"] = [0, 0]
        ...
        # Verify hp_active variable was created for load 1 (non-semi-cont)
        self.assertIn(1, self.opt.vars.get("hp_active", {}))
        # Verify load 0 uses p_def_bin2 (no hp_active created)
        self.assertNotIn(0, self.opt.vars.get("hp_active", {}))

    def test_heatpump_group_thermal_config_and_battery(self):
        """Test heatpump group with thermal_config (underfloor) and thermal_battery (hot water)."""
        self.optim_conf["number_of_deferrable_loads"] = 2
        self.optim_conf["nominal_power_of_deferrable_loads"] = [1000, 2000]
        self.optim_conf["operating_hours_of_each_deferrable_load"] = [0, 0]
        self.optim_conf["treat_deferrable_load_as_semi_cont"] = [True, False]
